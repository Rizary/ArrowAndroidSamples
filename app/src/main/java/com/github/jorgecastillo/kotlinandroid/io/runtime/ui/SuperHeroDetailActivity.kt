...
class SuperHeroDetailActivity : AppCompatActivity(), SuperHeroDetailView {
    ...
    intent.extras?.let {
      ...
      if (heroId == null) {
        ...
        unsafe {
          runNonBlocking({
            IO.runtime(ctx)
              .getSuperHeroDetails(heroId, this@SuperHeroDetailActivity)
          }, {})
        }
      }
    } ?: closeWithError()
  ...
  private fun closeWithError() {
    Toast.makeText(this, string.hero_id_needed, Toast.LENGTH_SHORT)
      .show()
  }
  ...
  override fun drawHero(hero: HeroViewState) {
    ...
    description.text =
      hero.description.let { if (it.isNotEmpty()) it else getString(string.empty_description) }
    headerImage.loadImageAsync(hero.photoUrl)
  }

  override fun showNotFoundError() {
    Snackbar.make(appBar, string.not_found, Snackbar.LENGTH_SHORT)
      .show()
  }

  override fun showGenericError() {
    Snackbar.make(appBar, string.generic, Snackbar.LENGTH_SHORT)
      .show()
  }

  override fun showAuthenticationError() {
    Snackbar.make(appBar, string.authentication, Snackbar.LENGTH_SHORT)
      .show()
  }
}
